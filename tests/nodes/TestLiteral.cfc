<cfcomponent extends="tests.TestCase" output="false">
	...
	<cffunction name="testInit" returntype="void" access="public">
		<cfscript>
			...
			assertFalse(StructKeyExists(loc.node, "subject"), "Literal should not have content until init() is called");
			loc.literal = loc.node.init("SELECT 1");
			assertSame(loc.node, loc.literal, "init() should return the same object");
			assertEquals("SELECT 1", loc.literal.subject, "Literal content should match that passed into constructor");
		</cfscript>
	</cffunction>
</cfcomponent>